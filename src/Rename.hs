...
renameUsProgs (UsProgExec tm) =
  pure UsProgExec <*> renameUsTm tm
renameUsProgs (UsProgFun x tp tm ps) =
  pure (UsProgFun x) <*> renameType tp <*> renameUsTm tm <*> renameUsProgs ps
renameUsProgs (UsProgExtern x tp ps) =
  pure (UsProgExtern x) <*> renameType tp <*> renameUsProgs ps
renameUsProgs (UsProgData y cs ps) =
  pure (UsProgData y) <*> mapM renameCtor cs <*> renameUsProgs ps

-- Alpha-rename a prog
renameProg :: Prog -> RenameM Prog
renameProg (ProgFun x ps tm tp) =
  bindVars ps (\ ps' -> pure (ProgFun x ps') <*> renameTerm tm) <*> renameType tp
renameProg (ProgExtern x xp ps tp) =
  pure (ProgExtern x) <*> bindVar xp return <*> mapM renameType ps <*> renameType tp
renameProg (ProgData y cs) =
  pure (ProgData y) <*> mapM renameCtor cs

-- Alpha-rename an entire program
renameProgs :: Progs -> RenameM Progs
renameProgs (Progs ps tm) =
  pure Progs <*> mapM renameProg ps <*> renameTerm tm

-- Auxiliary helper
alphaRename :: Ctxt -> RenameM a -> a
alphaRename g m =
  fst $ State.runState m $ ctxtToTermMap g

-- Auxiliary helper
alphaRenameUs :: Ctxt -> RenameUsM a -> a
alphaRenameUs g m =
  fst $ State.runState m $ ctxtToUsTmMap g

-- Alpha-rename a raw file
alphaRenameUsFile :: UsProgs -> Either String UsProgs
alphaRenameUsFile ps =
  return (alphaRenameUs (ctxtDefUsProgs ps) (renameUsProgs ps))

-- Alpha-rename an elaborated file
alphaRenameFile :: Progs -> Either String Progs
alphaRenameFile ps =
  return (alphaRename (ctxtDefProgs ps) (renameProgs ps))

-- Rename all occurrences of xi to xf in something
substs :: Ctxt -> [(Var, Either Term Type)] -> RenameM a -> a
substs g subs m =
  fst $ State.runState m $ foldr (uncurry Map.insert) (ctxtToTermMap g) $ map (fmap (either SubTm SubTp)) subs
...
lookupVar x =
  State.get >>= \ xs ->
  return (case Map.lookup x xs of { Just (SubVar x') -> x'; _ -> x })

lookupTerm :: Var -> (Var -> tm) -> RenameM' tm tm
lookupTerm x elsetm =
  State.get >>= \ xs ->
  return (case Map.lookup x xs of
            Just (SubTm tm) -> tm
            Just (SubVar x') -> elsetm x'
            _ -> elsetm x)
...
bindVar x f =
  ...
  f x' >>= \ a ->
  State.modify (maybe id (Map.insert x) (Map.lookup x xs)) >>
  return a
