...
TmApp (TmApp (TmVarG x []) tm1) tm2...   =>   TmVarG x [tm1, tm2, ...]
...
argifyFile (Progs ps tm) = Progs (map argifyProg ps) (argifyTerm tm) where
  ...
  arity (ProgFun x tp tm) = let (ls, etm) = splitLams tm in [(x, (snds ls, typeof etm))]
  ...
  argifyProg (ProgFun x tp tm) = ProgFun x tp (argifyTerm tm)
